"""Kling Provider：可灵 API 图生视频（预留）。

通过 Kling API 实现高质量 I2V，使用异步提交 + 轮询模式。
"""
...
_MODELS: list[dict] = [
    {"id": "kling-v1", "name": "Kling V1", "description": "标准质量"},
    {"id": "kling-v1-pro", "name": "Kling V1 Pro", "description": "高质量"},
]


class KlingProvider(BaseVideoGenProvider):
    """Kling 视频生成 Provider（预留）。

    当前版本为占位实现，调用时抛出 NotImplementedError。
    """
    ...
    def __init__(self, config: dict | None = None, **kwargs: object) -> None:
        ...
        self._api_base: str = cfg.get("api_base", "https://api.kling.ai")
        self._model: str = cfg.get("model", "kling-v1")
    ...
    async def _submit_task(
        ...
        if "error" in data:
            raise RuntimeError(f"Kling API 错误: {data['error']}")

        task_id = data.get("task_id") or data.get("id")
        if not task_id:
            raise RuntimeError(f"Kling 返回无效响应: {data}")
    ...
    async def _poll_task(self, client: httpx.AsyncClient, task_id: str) -> dict:
        ...
        while True:
            ...
            if elapsed > self._timeout:
                raise RuntimeError(f"Kling 任务超时: {task_id}")
            ...
            if status == "completed":
                return data
            if status == "failed":
                raise RuntimeError(f"Kling 任务失败: {data.get('error', '未知错误')}")
    ...
    async def generate(
        ...
        *,
        ...
        if not self._api_key:
            raise ValueError(
                "Kling API 密钥未配置，请在 config.toml 的 "
                "[video_gen.kling] 中设置 api_key"
            )
        ...
        if output_path is None:
            temp_dir = Path(tempfile.mkdtemp(prefix="kling_"))
        ...
        return VideoGenResult(
            ...
            height=height,
            fps=fps,
            metadata={"provider": "kling", "model": self._model, "task_id": task_id},
        )
